...
COMMENT ON COLUMN estufa.sensor.expected_interval_s IS 'Intervalo esperado entre leituras em segundos (60-86400)';
COMMENT ON COLUMN estufa.sensor.last_seen_at IS 'Timestamp da última telemetria recebida';

-- 2. Renomear tabela ambient_telemetry para telemetry
ALTER TABLE IF EXISTS estufa.ambient_telemetry RENAME TO telemetry;

-- 3. Recriar índice com novo nome da tabela
DROP INDEX IF EXISTS estufa.idx_ambient_telemetry_sensor_time;
CREATE INDEX IF NOT EXISTS idx_telemetry_sensor_time
  ON estufa.telemetry (sensor_id, received_at DESC);

-- 4. Atualizar last_seen_at com base nos dados existentes
UPDATE estufa.sensor s
SET last_seen_at = (
  SELECT MAX(t.received_at)
  FROM estufa.telemetry t
  WHERE t.sensor_id = s.id
)
WHERE EXISTS (
  SELECT 1 FROM estufa.telemetry t WHERE t.sensor_id = s.id
);
